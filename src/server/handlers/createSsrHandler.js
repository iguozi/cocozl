  ...
  return function serverSideResponse(req, res) {
    ...
    return createTimeout(ssrTimeout, Promise.all(promises))
      .then(() => {
        ...
        if (useBeta && req.hostname === 'cocozl.com') {
          return res.redirect(`http://cocozl.com${req.originalUrl}`);
        }
        ...
        return res.send(renderSsrPage(store, content, template, appUrl !== 'http://cocozl.com'));
      })
      .catch(err => {
